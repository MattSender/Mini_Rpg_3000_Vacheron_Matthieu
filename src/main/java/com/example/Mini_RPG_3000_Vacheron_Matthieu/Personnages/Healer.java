...
//----------------------- Création de la classe Healer contenants ses propres variables -----------------------//
public class Healer extends SpellCaster {
    public Healer(String name, int lifePoints, Team team){
        ...
        this.maxMana = 100;
        this.depenseMana = 20;
        this.msgAlerte = ("Oh non, ton \u2695 Guérisseur n'a plus de mana, il ne peut pas attaquer, fait quelque chose a l'aide, on est perdu !");
    ...
    public boolean attack(Combatant ennemi) {
        ...
        if(this.getMana()>= this.depenseMana){
            ennemi.setLifePoints(ennemi.getLifePoints()+soin);

            if(ennemi.getLifePoints()>this.getMaxLifePoints()){
                ennemi.setLifePoints(ennemi.getMaxLifePoints());
            }
